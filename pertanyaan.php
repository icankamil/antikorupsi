...
require_once "koneksi.php";
	...
	$limitData=limitData($halaman_awal, $batas);
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
  ...
  $idPertanyaan=uniqid('quiz');
  $namaPertanyaan = $_POST['namaPertanyaan'];
  $jenisPertanyaan = $_POST['jenisPertanyaan'];
  $dateCreate = date("Y-m-d");
  // id_produk bernilai '' karena kita set auto increment
  $q = tambahData($idPertanyaan,$namaPertanyaan,$jenisPertanyaan,$dateCreate);
  if ($q) {
    // pesan jika data tersimpan
    echo "<script>alert('Data berhasil ditambahkan'); window.location.href='pertanyaan.php'</script>";
  } else {
    // pesan jika data gagal disimpan
    echo "<script>alert('Data gagal ditambahkan');window.location.href='pertanyaan.php'</script>";
  }
}
...
//ubah data produk
if (isset($_POST['edit'])) {
  $namaPakan = $_POST['namaPertanyaan'];
  $merekPakan = $_POST['jenisPertanyaan'];
  $dateCreate = date("Y-m-d");
  $idPakan = $_POST['idPertanyaan'];
  $updateData = updateData($idPakan,$namaPakan,$merekPakan,$dateCreate);
}

if (isset($_POST['hapus'])) {
$id = $_POST["idPertanyaan"];
//mengambil id yang ingin dihapus
$hapusPakan=hapusData($id);
}
?>
						...
						<div class="col-lg-12">
							<div class="card flex-fill">
								<div class="card-header">
									...
									<button type="button" class="btn btn-success tambahPertanyaan" data-bs-toggle="modal" data-bs-target="#tambahModal">Tambah Data</button>
								</div>
...
while($row=mysqli_fetch_array($limitData)){
 ?> 
    <tr>
      <th><?php print($no++); ?></th>
      <td><?php print($row['namaPertanyaan']);?></td>
      <td><?php print($row['jenisPertanyaan']); ?></td>
	  <td><button type="button" id="<?php print($row['idPertanyaan'])?>" class="ubah-produk btn btn-warning" data-toggle="modal" data-target="#editModal"><i class="edit align-self-center"></i> Edit</button>
	  <button type="button" id="<?php print($row['idPertanyaan'])?>" class="hapus-produk btn btn-danger" data-toggle="modal" data-target="#hapusModal"><i class="delete align-self-center"></i> Hapus</button></td>
...
<div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
     <div class="row">
	 <div class="modal-body" id="dataProduk">
      </div>		
					</div>
	  </form>
    </div>
