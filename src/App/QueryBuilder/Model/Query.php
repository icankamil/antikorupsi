...
class Query
{
    ...
    public function generateSql()
    {
        ...
        $this->query['table'] .= $this->tableAlias != '' ? " AS " . $this->tableAlias . "" : "";
    ...
    public function queryWhereBetween(string $column, array $value, string $operator)
    {
        if ($this->query['where'] == "") {
            $this->query['where'] = " WHERE ";
        } elseif (substr($this->query['where'], -1) == "(" || substr($this->query['where'], -1) == ")") {
            $this->query['where'] .= "";
        } else {
            $this->query['where'] .= ' ' . $operator . ' ';
        }

        $this->query['where'] .= "$column BETWEEN '" . $value[0] . "' AND '" . $value[1] . "'";
    }

    public function whereBetween($column, $value)
    {
        $this->queryWhereBetween($column, $value, 'AND');
    ...
    {
        $this->queryWhereBetween($column, $value, 'OR');
